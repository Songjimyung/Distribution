...
class Test(APITestCase):
    ...
    def create_user(self, email, password):
        user = self.client.post(reverse("sign_up"), {"email": email,"name":"test1", "password":password, "re_password":password})
    ...
    def test_case_get_participating(self):
        ...
        for camp in self.camp_data:
            self.client.post(path=create_camp_url, data=camp, headers={"Authorization": token})
    ...
    def test_case_get_review(self):
        ...
        for review in review_data:
            self.client.post(path=create_review_url, data=review, headers={"Authorization": token})
        
        
        review = self.client.get(path=user_review_url, headers={"Authorization": token})
    ...
    def test_case_get_commend(self):
        ...
        for commend in commend_data:
            status = self.client.post(path=create_commend_url, data=commend, headers={"Authorization": token})
        
        comment = self.client.get(path=user_commned_url, headers={"Authorization": token})
